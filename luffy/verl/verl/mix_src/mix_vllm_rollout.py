        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'tgt_input_ids' in prompts.batch: # in train mode
                    ...
                    if prompts.meta_info.get('force_zero_prefix', None):
                        # 这里如果什么也没有get到会有错误
                        print("[采样调试] 强制使用零前缀采样")
                        self.config.prefix_strategy = 'zeroPrefixRatio'
                    else:
                        print("[采样调试] 使用Luffy原有的采样策略")
                        self.config.prefix_strategy = "random"

                    import random
                    print(f"[采样调试策略确认] prefix_strategy: {self.config.prefix_strategy}")
                    print(f"[采样调试] n_prefix: {self.config.n_prefix}")
                    print(f"[采样调试] prefix_share_across_samples: {self.config.prefix_share_across_samples}")
                    ...
                    if not self.config.prefix_share_across_samples:
                        ...
                        if self.config.n_prefix == -1:
                            ...
                            elif self.config.prefix_strategy == 'zeroPrefixRatio':
                                prefix_ratios = [0.0] * len(tgt_list)
                                # print("prefix ratios已设置为全0")
                            ...
                            for i in range(batch_size):
                                if self.config.prefix_strategy == 'random':
                                    ...
                                    prefix_ratios.extend([0.0] * self.config.n_prefix)
                                    # print("[采样调试]prefix ratios已设置为全0，n_prefix: ", self.config.n_prefix)
                                    # 实际上目前调试过的是这条通路
                                elif self.config.prefix_strategy == 'reverse_linear' or self.config.prefix_strategy == 'linear_max':
                                    w_idx = -1
                                    for i in range(len(self.prefix_step_windows)):
